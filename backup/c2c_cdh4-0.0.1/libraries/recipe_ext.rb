...
#			puts e.message
    ...
    def find_cloudera_namenode(environment = nil)
       ...
       # if node.run_list.expand(environment).recipes.include?("c2c_cdh4:hadoop_namenode")
        if node.recipes.include?("c2c_cdh4::hadoop_namenode")
          node
        else
          search_string = "recipes:c2c_cdh4\\:\\:hadoop_namenode"
    ...
    def find_cloudera_jobtracker(environment = nil)
        if node.recipes.include?("c2c_cdh4::hadoop_jobtracker")
          node
        else
          search_string = "recipes:c2c_cdh4\\:\\:hadoop_jobtracker"
          search_string << " AND chef_environment:#{environment}" if environment
          search(:node, search_string) do |matching_node|
...
# Don't use function below, search seems to be broken (HTTP Request Returned 400 Bad Request: invalid search query: 'recipes:"c2c_cdh4::hadoop_jobtracker"' Parse error at offset: 7 Reason: Expected one of \, " at line 1, column 18 (byte 18) after )
