...
include_recipe "c2c_cloudera::repo"
...
namenode_servers = search(:node, "chef_environment:#{node.chef_environment} AND recipes:c2c_cloudera\\:\\:hadoop_namenode OR recipes:c2c_cloudera\\:\\:hadoop_secondary_namenode")
masters = namenode_servers.map { |node| node[:ipaddress] }
datanode_servers = search(:node, "chef_environment:#{node.chef_environment} AND recipes:c2c_cloudera\\:\\:hadoop_datanode")
